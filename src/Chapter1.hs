...
🕯 HINT: Yo can use the 'abs' function and the __if-then-else__ Haskell syntax
  for this task.
...
🕯 HINT: When checkiing multiple conditions, it is more convenient to use the
...
sumLast2 n = last + nextLast
  where last = n `mod` 10
        nextLast = (n `div` 10) `mod` 10
